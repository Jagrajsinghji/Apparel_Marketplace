...
class _CategoriesPageState extends State<CategoriesPage> {
  ...
  void initState() {
    super.initState();
    Timer(Duration(milliseconds: 500),()=>getProducts(false));
  }
    ...
    prodsData.sort((a, b) {
      switch (sort) {
        ...
        case Sort.Popularity:
          ...
          break;
        case Sort.Discount:
          double newPrice1 = double.parse(a['price']?.toString());
          double prevPrice1 =
              double.parse(a['previous_price']?.toString() ?? "0");
          int discount1 = 0;
          if (prevPrice1 > 0)
            discount1 = (((prevPrice1 - newPrice1) / prevPrice1) * 100).round();
          double newPrice2 = double.parse(b['price']?.toString());
          double prevPrice2 =
              double.parse(b['previous_price']?.toString() ?? "0");
          int discount2 = 0;
          if (prevPrice2 > 0)
            discount2 = (((prevPrice2 - newPrice2) / prevPrice2) * 100).round();
